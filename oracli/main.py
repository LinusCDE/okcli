...
from .config import (write_default_config, get_cnf_path,
                     open_cnf, read_config_files, str_to_bool)
...
class OraCli(object):
    ...
    def __init__(self, sqlexecute=None, prompt=None,
        ...
        mylogin_cnf_path = get_cnf_path()
        if mylogin_cnf_path:
            mylogin_cnf = open_cnf(mylogin_cnf_path)
    ...
    def connect(self, database='', user='', passwd='', host='', port='',
        ...
        try:
            try:
                sqlexecute = SQLExecute(database, user, passwd, host, port, charset)
...
@click.option('-P', '--port', type=int, help='Port number to use for connection.')
...
def cli(sqlplus, user, host, port, password, database,
    ...
    oracli.connect(database, user, password, host, port)
