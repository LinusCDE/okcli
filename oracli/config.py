...
def get_cnf_path():
    ...
    oracle_login_path = os.getenv('SQLPATH')

    if oracle_login_path is None:
        app_data = os.getenv('APPDATA')
        default_dir = os.path.join(app_data, 'ORACLE') if app_data else '~'
        oracle_login_path = os.path.join(default_dir, '.mylogin.cnf')

    oracle_login_path = os.path.expanduser(oracle_login_path)

    if exists(oracle_login_path):
        logger.debug("Found login path file at '{0}'".format(oracle_login_path))
        return oracle_login_path
    return None
